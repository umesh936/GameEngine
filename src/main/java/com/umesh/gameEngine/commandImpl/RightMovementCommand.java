...
public class RightMovementCommand implements Command {
	...
	public String executeAndRespond() throws ConfigurationInitializationException {
		...
		Move move = new Move();
		move.setMoveName(GameConstants.MoveConstant.RIGHT);
		Game.getInstance().getGameState().getCurrentPlayer().addmoveTohistory(move);
		if (dangerMove.equals(GameConstants.MoveConstant.RIGHT)) {
			move.setDangerMove();
