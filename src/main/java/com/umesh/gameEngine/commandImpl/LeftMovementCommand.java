...
public class LeftMovementCommand implements Command {
	...
	public String executeAndRespond() throws ConfigurationInitializationException {
		...
		Move move = new Move();
		move.setMoveName(GameConstants.MoveConstant.LEFT);
		Game.getInstance().getGameState().getCurrentPlayer().addmoveTohistory(move);
		if (dangerMove.equals(GameConstants.MoveConstant.LEFT)) {
			move.setDangerMove();
